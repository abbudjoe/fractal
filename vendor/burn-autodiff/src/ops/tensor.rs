...
fn should_use_skinny_rank2_weight_grad<B: Backend>(
    lhs: &B::FloatTensorPrimitive,
    grad: &B::FloatTensorPrimitive,
) -> bool {
    const MAX_BATCH: usize = 8;
    const MAX_TEMP_ELEMENTS: usize = 8 * 1024 * 1024;

    let lhs_shape = lhs.shape();
    let grad_shape = grad.shape();

    if lhs_shape.num_dims() != 2 || grad_shape.num_dims() != 2 {
        return false;
    }

    let batch = lhs_shape[0];
    let d_input = lhs_shape[1];
    let grad_batch = grad_shape[0];
    let d_output = grad_shape[1];

    if batch == 0 || batch != grad_batch || batch > MAX_BATCH {
        return false;
    }

    batch
        .checked_mul(d_input)
        .and_then(|value| value.checked_mul(d_output))
        .is_some_and(|elements| elements <= MAX_TEMP_ELEMENTS)
}

fn skinny_rank2_weight_grad<B: Backend>(
    lhs: B::FloatTensorPrimitive,
    grad: B::FloatTensorPrimitive,
) -> B::FloatTensorPrimitive {
    let lhs_shape = lhs.shape();
    ...
    let lhs = B::float_reshape(lhs, Shape::new([batch, d_input, 1]));
    let grad = B::float_reshape(grad, Shape::new([batch, 1, d_output]));
    let product = B::float_mul(lhs, grad);
    let summed = B::float_sum_dim(product, 0);

    B::float_reshape(summed, Shape::new([d_input, d_output]))
}
        ...
        impl<B: Backend> Backward<B, 2> for Matmul {
            ...
            fn backward(
                ...
                binary::<B, _, _>(
                    ...
                    |grad| {
                        let lhs = lhs.unwrap();
                        let grad = if should_use_skinny_rank2_weight_grad::<B>(&lhs, &grad) {
                            skinny_rank2_weight_grad::<B>(lhs, grad)
                        } else {
                            let lhs = B::float_transpose(lhs);
