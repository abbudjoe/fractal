...
## Causal Memory Auditor (Counterfactual Memory Credit Assignment)

v1 should not only log what memory paths were selected.

It should also estimate which memory paths were **causally useful**.

This architecture is especially vulnerable to decorative complexity:

- the tree may exist but do little real work
- routing may look structured while not affecting outcomes
- exact leaf read may dominate all useful retrieval
- the local trunk may silently carry the full model
- extra roots may add width without adding differentiated function

To prevent that, v1 should include a **Causal Memory Auditor**.

### Goal

Estimate the contribution of each major memory path by running cheap counterfactual interventions on a sampled subset of positions, batches, or evaluation examples.

The purpose is to measure:

- whether routed tree summaries matter
- whether exact leaf read matters
- whether a selected span matters relative to nearby alternatives
- whether individual roots contribute distinct utility
- whether the architecture is using memory as intended rather than merely carrying it as decoration

### Core method

For a sampled subset of training or evaluation steps:

1. run the normal forward pass
2. record selected roots, routed spans, and exact leaf-read targets
3. run one or more local counterfactual variants
4. compare the perturbed outputs to the full forward pass

The auditor does not need to run on every step.

It should be a sampled analysis path used often enough to reveal collapse, redundancy, and dead-weight components.

### Required counterfactual interventions

The first version should support at least:

- **No tree read**  
  Zero or remove retrieved tree-summary values while leaving the rest of the forward path unchanged.

- **No exact leaf read**  
  Remove the exact local read path while preserving routed coarse summaries.

- **Next-best span substitution**  
  Replace a selected routed span with the next-best candidate span at the same routing depth.

- **Root dropout intervention**  
  Remove one root contribution at a time from the fused readout.

Optional later interventions may include:

- random same-depth span substitution
- shallow-only retrieval vs deep-only retrieval
- coarse-summary-only vs leaf-summary-plus-exact-read
- selected-head removal

### Metrics

For each intervention, record deltas such as:

- loss difference
- target-token logit difference
- KL divergence from the full forward output distribution
- retrieval accuracy difference on synthetic tasks
- perplexity difference on sampled evaluation spans
...
### Minimum v1 reporting surfaces

The auditor should produce at least:

- average utility of tree retrieval vs no-tree baseline
- average utility of exact leaf read vs no-exact-read baseline
...
- utility by task family:
  ...
  - induction
  - noisy retrieval
  - ordinary next-token LM evaluation

### Why this is required

This architecture can fail in ways that ordinary diagnostics do not expose.

Examples:

- routing entropy looks healthy, but removing tree retrieval changes nothing
- roots appear diverse, but dropping any one root has near-zero effect
- exact leaf read accounts for nearly all retrieval utility
- the local trunk carries the model while the tree becomes dead weight
- routed spans are stable but not causally useful

The Causal Memory Auditor exists to make those failures visible early.

### v1 operating policy

The auditor should be:

- sampled, not full-time
- cheap enough to run during regular evaluation
- optionally enabled during training
- mandatory for architecture validation runs

The first implementation should prefer **small, explicit perturbations** over sophisticated attribution methods.

This is a measurement system, not a learned policy.

### Success conditions

The auditor supports the architecture if it shows patterns like:

- routed tree summaries measurably improve outputs
- exact leaf read helps, but is not the only useful component
- different roots show differentiated utility
- deeper retrieval is useful on tasks that require it
- memory-path utility varies meaningfully by task type

### Failure conditions

The auditor is evidence against the current architecture if it repeatedly shows that:

- removing tree retrieval barely changes outputs
- exact leaf read is the only memory path with meaningful utility
- root contributions are nearly interchangeable
- deeper routing has negligible causal value
- sparse retrieval adds complexity without measurable benefit

### Implementation note

The auditor should be introduced as a **first-class validation surface**, not an afterthought.

It is part of the architecture plan because this model’s central claim is not merely that memory exists, but that **specific memory paths do useful work**.

### Recommendation

Add the Causal Memory Auditor immediately after the first sparse-retrieval and exact-read path is stable.

Do not defer it until late-stage training.

By the time large runs begin, the project should already be able to answer:

- which memory path changed the prediction
- by how much
- on what kind of task
- at what depth
- and whether the extra architectural complexity is paying for itself
...
The Causal Memory Auditor exists to make those failure modes measurable rather than interpretive.
