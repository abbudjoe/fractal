...
pub fn default_v2_synthetic_probe_suites() -> Vec<SyntheticProbeSuite> {
    v2_synthetic_probe_suites_for_leaf_size(16)
        .expect("default v2 synthetic probe suites must validate for leaf_size=16")
}

pub fn v2_synthetic_probe_suites_for_leaf_size(
    leaf_size: usize,
) -> Result<Vec<SyntheticProbeSuite>, FractalError> {
    Ok(vec![
        copy_probe_suite_for_leaf_size(leaf_size)?,
        associative_recall_probe_suite_for_leaf_size(leaf_size)?,
        induction_probe_suite_for_leaf_size(leaf_size)?,
        noisy_retrieval_probe_suite_for_leaf_size(leaf_size)?,
        far_token_comparison_probe_suite_for_leaf_size(leaf_size)?,
    ])
}
...
fn copy_probe_suite() -> SyntheticProbeSuite {
    copy_probe_suite_for_leaf_size(16).expect("default copy probe suite must validate")
}

fn copy_probe_suite_for_leaf_size(leaf_size: usize) -> Result<SyntheticProbeSuite, FractalError> {
    Ok(SyntheticProbeSuite {
        ...
        samples: vec![
            SyntheticProbeSample::final_target(
                ...
                "copy-alpha",
                expand_probe_input_for_leaf_size(
                    &[
                        COPY_SENTINEL,
                        41,
                        12,
...
fn associative_recall_probe_suite_for_leaf_size(
    leaf_size: usize,
) -> Result<SyntheticProbeSuite, FractalError> {
    Ok(SyntheticProbeSuite {
...
fn induction_probe_suite_for_leaf_size(
    leaf_size: usize,
) -> Result<SyntheticProbeSuite, FractalError> {
    Ok(SyntheticProbeSuite {
...
fn noisy_retrieval_probe_suite() -> SyntheticProbeSuite {
    noisy_retrieval_probe_suite_for_leaf_size(16)
        .expect("default noisy retrieval probe suite must validate")
}

fn noisy_retrieval_probe_suite_for_leaf_size(
    leaf_size: usize,
) -> Result<SyntheticProbeSuite, FractalError> {
    Ok(SyntheticProbeSuite {
...
fn far_token_comparison_probe_suite_for_leaf_size(
    leaf_size: usize,
) -> Result<SyntheticProbeSuite, FractalError> {
    Ok(SyntheticProbeSuite {
...
fn expand_probe_input_for_leaf_size(
    base_input_ids: &[i64],
    leaf_size: usize,
) -> Result<Vec<i64>, FractalError> {
    ensure_nonzero("synthetic_probe_suite.leaf_size", leaf_size, "")?;
    if leaf_size < 16 {
        return Err(FractalError::InvalidConfig(format!(
            "synthetic_probe leaf_size {} must be at least 16 to preserve the canned evidence spans",
            leaf_size
        )));
    }
    if base_input_ids.len() < 2 {
        return Err(FractalError::InvalidConfig(
            "synthetic_probe base input must contain at least two suffix tokens".to_string(),
        ));
    }

    let minimum_len = leaf_size.checked_mul(2).ok_or_else(|| {
        FractalError::InvalidConfig(format!(
            "synthetic_probe leaf_size {} overflowed the two-leaf minimum",
            leaf_size
        ))
    })?;
    if base_input_ids.len() >= minimum_len {
        return Ok(base_input_ids.to_vec());
    }

    let filler_len = minimum_len - base_input_ids.len();
    let split_index = base_input_ids.len() - 2;
    let mut expanded = Vec::with_capacity(minimum_len);
    expanded.extend_from_slice(&base_input_ids[..split_index]);
    for index in 0..filler_len {
        expanded.push(7 + (index % 8) as i64);
    }
    expanded.extend_from_slice(&base_input_ids[split_index..]);
    Ok(expanded)
}
...
mod tests {
    ...
    #[test]
    fn synthetic_probe_suites_expand_cleanly_for_larger_leaf_sizes() {
        let suites = v2_synthetic_probe_suites_for_leaf_size(32).unwrap();

        assert!(suites.iter().all(|suite| suite.leaf_size == 32));
        for suite in &suites {
            suite.validate_for_model(64, 32).unwrap();
            assert!(suite
                .samples
                .iter()
                .all(|sample| sample.input_ids.len() >= 64 && sample.target_position >= 32));
        }
    }
