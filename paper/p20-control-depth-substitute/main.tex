...
\newcommand{\pcontrol}{P20-control}
...
\pcontrol{}, inserted into a looped middle-depth causal language-model scaffold.
...
\pcontrol{} beats the strongest non-P20 looped control across three seeds,
survives a speed-compensated control check, and remains ahead after a bounded
learning-rate sweep of non-P20 looped baselines. Against pure attention,
...
causal decoder. The primitive is called \pcontrol{}. It is not inserted as a
full attention replacement. It controls the injection path of a looped
middle-depth scaffold, where a small set of middle attention blocks is reused
for multiple recurrent passes.
...
\begin{description}
  ...
  \item[\parcae{} \pcontrol{} looped attention.] The same scaffold with a
    P20 recurrent scan over the prelude stream. The P20 output is normalized
    and used to produce the injected value and gate for the middle loop.
\end{description}
...
\parcae{} \pcontrol{} looped attention & 9.87M & 4.3012 & 793,615 & 7,046.88 \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
Seed & \bx{} loss & \pcontrol{} loss & Delta & \bx{} tok/s & \pcontrol{} tok/s \\
\midrule
...
control advantage over the strongest non-P20 looped injection path.
...
\subsection{Tuned non-P20 controls}

We next gave non-P20 looped controls a bounded learning-rate sweep on seed 42:
3e-4, 6e-4, 1e-3, and 1.5e-3. The best non-P20 setting was plain \parcae{}
looped attention at LR 6e-4. It still lost to \pcontrol{} across three seeds.

\begin{table}[h]
\centering
\caption{Seed-42 learning-rate sweep for non-P20 looped controls.}
\label{tab:lrsweep}
\begin{tabular}{rrr}
\toprule
...
\begin{table}[h]
\centering
\caption{Best tuned non-P20 control versus \pcontrol{}.}
\label{tab:tuned}
\begin{tabular}{rrrrr}
\toprule
...
\begin{tabular}{rrrrrr}
\toprule
Seed & 14L loss & \pcontrol{} loss & Delta & 14L tok/s & \pcontrol{} tok/s \\
\midrule
...
\begin{itemize}
  \item It reliably beats same-scaffold non-P20 controls.
...
\begin{itemize}
  ...
  \item optionally a looped non-P20 control if budget allows.
\end{itemize}
...
improves a looped middle-depth attention scaffold beyond matched non-P20
