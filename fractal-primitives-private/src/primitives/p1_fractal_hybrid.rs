...
impl<B: Backend> FractalRule<B> for P1FractalHybrid<B> {
    fn apply(
        ...
        let squared =
            clamp_symmetric_by_row(previous_state.clone() * previous_state.clone(), clamp_val);
        let c_t = self.u.forward(x.clone());
        let main_update = self.w_h.forward(previous_state.clone()) + c_t + squared;
        let next = g.clone() * main_update + one_minus(g) * previous_state;
