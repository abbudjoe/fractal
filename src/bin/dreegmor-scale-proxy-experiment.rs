...
use fractal_core::{
    ...
    scale_proxy_recurrent_over_attention_only_variant, CpuTrainBackend, MetalTrainBackend,
};
use fractal_eval_private::{
    byte_level_smoke_corpus_stats_from_source, default_v3a_fineweb_stage0_canary_corpus_source,
    run_attention_only_hybrid_attention_smoke_train,
    run_attention_only_recurrent_scale_proxy_smoke_train,
    run_attention_only_scale_proxy_smoke_train, ByteLevelSmokeCorpusSource,
...
const EXPERIMENT_TITLE: &str = "DREEGMOR Scale Proxy Experiment";
const EXPERIMENT_SLUG: &str = "dreegmor-scale-proxy-experiment";
    ...
    B: burn::tensor::backend::AutodiffBackend,
{
    ...
    let one_shot_variant = scale_proxy_one_shot_over_attention_only_variant();
    let recurrent_variant = scale_proxy_recurrent_over_attention_only_variant();
    ...
    if args.variant.includes_one_shot_scale_proxy() {
        let label = one_shot_variant.label.clone();
        let report = run_attention_only_scale_proxy_smoke_train::<B>(
            scale_proxy_smoke_config(
                corpus_source.clone(),
        ...
        .map_err(|error| format!("failed to run DREEGMOR scale proxy over A: {error}"))?;
        variants.push(ExperimentVariantSummary::routed(
            "DREEGMOR-ScaleProxy(A)",
            report.config.execution_backend,
            report.config.variant.label.clone(),
            report.model_label,
            report.note,
    ...
    if args.variant.includes_recurrent_scale_proxy() {
        let label = recurrent_variant.label.clone();
        let report = run_attention_only_recurrent_scale_proxy_smoke_train::<B>(
            recurrent_scale_proxy_smoke_config(
                corpus_source,
                isolated_variant_dir(output_dir, &label),
                recurrent_variant,
                args,
        ...
        .map_err(|error| format!("failed to run recurrent DREEGMOR scale proxy over A: {error}"))?;
        variants.push(ExperimentVariantSummary::routed(
            "DREEGMOR-ScaleProxy-Recurrent(A)",
            report.config.execution_backend,
            report.config.variant.label.clone(),
            report.model_label,
            report.note,
...
fn scale_proxy_smoke_config(
    corpus_source: ByteLevelSmokeCorpusSource,
    output_dir: PathBuf,
    variant: fractal_core::AttentionOnlyScaleProxyVariantSpec,
    args: &CliArgs,
) -> ScaleProxySmokeTrainConfig {
    let mut config = ScaleProxySmokeTrainConfig::new(corpus_source, output_dir, variant);
    config.execution_backend = args.backend.execution_backend();
    config.seq_len = args.seq_len;
    config.window_stride = args.window_stride.unwrap_or(args.seq_len);
    config.batch_size = args.batch_size;
...
fn recurrent_scale_proxy_smoke_config(
    corpus_source: ByteLevelSmokeCorpusSource,
    output_dir: PathBuf,
    variant: fractal_core::AttentionOnlyRecurrentScaleProxyVariantSpec,
    args: &CliArgs,
) -> RecurrentScaleProxySmokeTrainConfig {
    let mut config = RecurrentScaleProxySmokeTrainConfig::new(corpus_source, output_dir, variant);
    config.execution_backend = args.backend.execution_backend();
    config.seq_len = args.seq_len;
    config.window_stride = args.window_stride.unwrap_or(args.seq_len);
    config.batch_size = args.batch_size;
    ...
    if let (Some(train_path), Some(eval_path)) = (&args.jsonl_train_path, &args.jsonl_eval_path) {
        let corpus_name = args.corpus_name.clone().unwrap_or_else(|| {
            train_path
                ...
                .unwrap_or("dreegmor-scale-proxy-jsonl-split")
                .to_owned()
        });
...
impl VariantSelection {
    fn parse(value: &str) -> Result<Self, String> {
        match value {
            ...
            "one-shot-a" | "scale-proxy-a" | "dreegmor-scale-proxy-a" => {
                Ok(Self::OneShotScaleProxy)
            }
            "recurrent-a" | "recurrent-scale-proxy-a" | "dreegmor-scale-proxy-recurrent-a" => {
                Ok(Self::RecurrentScaleProxy)
            }
    ...
    fn includes_one_shot_scale_proxy(self) -> bool {
        matches!(self, Self::All | Self::OneShotScaleProxy)
    }

    fn includes_recurrent_scale_proxy(self) -> bool {
        matches!(self, Self::All | Self::RecurrentScaleProxy)
    }
}
...
struct ExperimentVariantSummary {
    ...
    report_path: PathBuf,
    routing: Option<ScaleProxyRoutingSummary>,
}
...
fn render_report(
    ...
    match output {
        OutputFormat::Json => serde_json::to_string_pretty(summaries)
            ...
            .map_err(|error| format!("failed to serialize scale-proxy report: {error}")),
...
fn render_table(summaries: &[ExperimentVariantSummary]) -> String {
    ...
    let _ = writeln!(
        output,
        "note: shared-trunk internal-expertization lane. variants run sequentially and write to isolated subdirectories; use single-variant invocations for memory comparisons."
    );
    for summary in summaries {
        ...
        if let Some(routing) = &summary.routing {
            let _ = writeln!(
                ...
                routing.mean_initial_channel_weights[0],
                routing.mean_initial_channel_weights[1],
                routing.mean_final_channel_weights[0],
                routing.mean_final_channel_weights[1],
            ...
            let _ = writeln!(
                output,
                "  routing_entropy_bits={:.3} winner_margin={:.3} controller_adjustment_l1={:.3} per_round_l1={:?} sampled_tokens={}",
                routing.mean_route_entropy_bits,
                routing.mean_winner_margin,
                routing.mean_controller_adjustment_l1,
...
fn usage() -> String {
    format!(
        "{EXPERIMENT_TITLE}\n\
         ...
         Options:\n\
           --variant <all|a|scale-proxy-a|recurrent-scale-proxy-a>\n\
           --backend <cpu|metal>\n\
           --steps <n>\n\
           --eval-batches <n>\n\
           --full-train-pass\n\
           ...
           --window-stride <n>\n\
           --batch-size <n>\n\
           --eval-holdout-every <n>\n\
           --learning-rate <f64>\n\
           --seed <u64>\n\
           --data-seed <u64>\n\
           --corpus-path <path> (repeatable)\n\
           --jsonl-train-path <path>\n\
           --jsonl-eval-path <path>\n\
           --corpus-name <name>\n\
           --corpus-text-field <field>\n\
           --output-dir <path>\n\
           --output <table|json>\n\
           --help\n"
    )
