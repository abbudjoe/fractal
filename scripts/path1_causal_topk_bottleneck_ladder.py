...
SUITE_NAME = "path1-causal-topk-bottleneck-ladder-v1"
...
def _lanes() -> tuple[LaneSpec, ...]:
    return (
        LaneSpec(
            ...
            family="control",
            note="Dense 8-layer attention control.",
            build_variant=lambda shape: phase1_attention_only_variant(shape=shape),
        ),
        LaneSpec(
            name="mod-train-topc-route50-layer3",
            family="paper MoD train-time reference",
            note="Full-sequence top-C MoD reference at the Parcae middle-band entry layer.",
        ...
        LaneSpec(
            ...
            family="placement",
            note="Early hard causal prefix-top-k routing.",
            build_variant=lambda shape: phase1_attention_only_variant(
                shape=shape,
        ...
        LaneSpec(
            ...
            family="placement",
            note="Middle-band-entry hard causal prefix-top-k routing.",
            build_variant=lambda shape: phase1_attention_only_variant(
                shape=shape,
                token_routing_profile=TokenRoutingProfile.CAUSAL_TOPK_BLOCK,
                token_route_fraction=0.5,
        ...
        LaneSpec(
            name="causal-topk-route50-layer6",
            family="placement",
            note="Late hard causal prefix-top-k routing.",
            build_variant=lambda shape: phase1_attention_only_variant(
                shape=shape,
                token_routing_profile=TokenRoutingProfile.CAUSAL_TOPK_BLOCK,
                token_route_fraction=0.5,
                token_routing_layer_indices=(6,),
            ),
        ),
        LaneSpec(
            name="causal-topk-route25-layer3",
            family="hardness",
            note="More aggressive middle-layer hard skip.",
            build_variant=lambda shape: phase1_attention_only_variant(
                shape=shape,
                token_routing_profile=TokenRoutingProfile.CAUSAL_TOPK_BLOCK,
        ...
        LaneSpec(
            name="causal-topk-route75-layer3",
            family="hardness",
            note="Less aggressive middle-layer hard skip.",
            build_variant=lambda shape: phase1_attention_only_variant(
                shape=shape,
                token_routing_profile=TokenRoutingProfile.CAUSAL_TOPK_BLOCK,
                token_route_fraction=0.75,
                token_routing_layer_indices=(3,),
            ),
        ),
    )
...
def _row(
    *, lane: LaneSpec, seed: int, report_path: Path, report: dict[str, object]
    ...
    return {
        ...
        "selected_first_half_fraction": _float_payload(
            routing, "last_selected_first_half_fraction"
        ),
        "selected_second_half_fraction": _float_payload(
            routing, "last_selected_second_half_fraction"
        ),
        "mean_selected_position": _float_payload(
            routing, "last_mean_selected_position"
        ),
        ...
        aggregate.append(
            {
                ...
                "mean_selected_first_half_fraction": _maybe_mean(
                    group, "selected_first_half_fraction"
                ),
                "mean_selected_second_half_fraction": _maybe_mean(
                    group, "selected_second_half_fraction"
                ),
                "mean_selected_position": _maybe_mean(
                    group, "mean_selected_position"
                ),
...
def _write_summary(
    ...
    lines = [
        f"# {SUITE_NAME}",
        "",
        "Targeted ladder for the hard-skip and prefix-skew hypotheses in causal top-k routing.",
        "",
        "## Conditions",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Aggregate",
            "",
            "| Lane | Family | Loss | Loss Std | Tok/s | Params | Selected | First Half | Second Half | Mean Pos |",
            "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        ]
    )
    for row in aggregate:
        lines.append(
            "| {lane} | {family} | {loss:.4f} | {std:.4f} | {speed:.1f} | {params} | {selected} | {first} | {second} | {position} |".format(
                lane=row["lane"],
                family=row["family"],
                ...
                first=_format_optional(row["mean_selected_first_half_fraction"]),
                second=_format_optional(row["mean_selected_second_half_fraction"]),
                position=_format_optional(row["mean_selected_position"]),
            )
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run a targeted causal top-k routing bottleneck ladder."
    )
