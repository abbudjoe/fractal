...
if [[ "${PYTHON_INSTALL_MODE}" != "official-mamba3" ]]; then
  echo "smoke expects PYTHON_INSTALL_MODE=official-mamba3 because it includes native reference-ssm runs"
  exit 1
fi
if [[ "${PRIMITIVE_RUNTIME_BACKEND}" != "torch" ]]; then
  echo "smoke currently expects PRIMITIVE_RUNTIME_BACKEND=torch"
  exit 1
fi
