#!/usr/bin/env bash
# Run longer matched MaxText seed replication for attention control versus the
# layer-1 zero-init RGRP MLP sidecar.
#
...
OUTDIR="${2:-$HOME/fractal_sidecar_zero_long_seeds_${STAMP}}"
SEEDS="${SEEDS:-1 2}"
STEPS="${STEPS:-1000}"
...
SUMMARY_FILTER='/number parameters:|Memstats: After params initialized:|Using \\(GB\\)|eval metrics after step|completed step: (0|999),/ { print; fflush(); }'
