...
STEPS="${2:-128}"
EVAL_BATCHES="${3:-1}"
LABEL_PREFIX="${4:-v3a-python-path1-gdnp-fused-law-ablation}"

GPU_ID="${GPU_ID:-NVIDIA GeForce RTX 5090}"
CLOUD_TYPE="${CLOUD_TYPE:-SECURE}"
POD_NAME="${POD_NAME:-fractal-v3a-gdnp-fused-law-5090-s${SEED}}"
...
PROFILES=(
  gated-deltanet-p20-fused-torch
  gated-deltanet-p20-fused-beta-torch
  gated-deltanet-p20-fused-qkv-torch
  gated-deltanet-p20-fused-residual-readout-torch
  gated-deltanet-p20-fused-multi-read-torch
  gated-deltanet-p20-fused-all-torch
)
...
echo "completed fused GDN/P20 law ablation seed ${SEED}"
