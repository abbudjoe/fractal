...
SEED="${1:-42}"
LABEL_PREFIX="${2:-v3a-python-path1-profile}"
...
PROFILE_ROW_LIMIT="${PROFILE_ROW_LIMIT:-30}"
...
if [[ "${PYTHON_INSTALL_MODE}" != "official-mamba3" ]]; then
  echo "profile expects PYTHON_INSTALL_MODE=official-mamba3 because it includes native reference-ssm profiling"
  exit 1
fi
if [[ "${PRIMITIVE_RUNTIME_BACKEND}" != "torch" ]]; then
  echo "profile currently expects PRIMITIVE_RUNTIME_BACKEND=torch"
  exit 1
fi
...
echo "completed runpod v3a python path1 profile seed ${SEED}"
