...
SEED="${1:-42}"
LABEL_PREFIX="${2:-v3a-python-path1-packed-inproj-sweep}"

POD_NAME_PREFIX="${POD_NAME_PREFIX:-fractal-v3a-packed-inproj}"
...
PRIMITIVE_PROFILES="${PRIMITIVE_PROFILES:-p1,p2,p2-1,p2-2,p2-3}"
WARMUP_EVAL_BATCHES="${WARMUP_EVAL_BATCHES:-1}"
WARMUP_TRAIN_STEPS="${WARMUP_TRAIN_STEPS:-1}"

if [[ "${PYTHON_INSTALL_MODE}" != "compile-safe" ]]; then
  echo "packed in_proj primitive sweep expects PYTHON_INSTALL_MODE=compile-safe"
  exit 1
fi
...
run_lane() {
  ...
  local run_label="${LABEL_PREFIX}-s${SEED}-${primitive_profile}-scaled-projected-pre-norm-only-standard-state-${PRIMITIVE_STATE_TRANSFORM_PROFILE}-env-${PYTHON_INSTALL_MODE}-compile-${COMPILE_MODE}"
...
echo "completed runpod v3a python path1 packed in_proj sweep seed ${SEED}"
