...
STEPS="${2:-128}"
EVAL_BATCHES="${3:-1}"
LABEL_PREFIX="${4:-v3a-python-path1-composite-swa-smoke}"

GPU_ID="${GPU_ID:-NVIDIA GeForce RTX 5090}"
CLOUD_TYPE="${CLOUD_TYPE:-SECURE}"
POD_NAME="${POD_NAME:-fractal-v3a-composite-swa-5090-s${SEED}}"
...
INSTALL_MODE="${INSTALL_MODE:-official-mamba3}"
...
PROFILES=(
  ...
  p20-mamba3-torch
  gated-deltanet-mamba3-torch
  gated-deltanet-p20-mamba3-torch
)
...
echo "completed composite SWA smoke seed ${SEED}"
