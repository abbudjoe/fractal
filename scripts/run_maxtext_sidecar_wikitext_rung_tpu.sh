#!/usr/bin/env bash
# Run a larger-text TPU/JAX rung for attention vs RGRP vs the strongest
# matched extra-capacity control.
#
# Execute inside a patched MaxText checkout. WikiText-103 is used here because
# it is MaxText-readable through HF streaming and has a real validation split.

set -euo pipefail

STAMP="${1:-$(date -u +%Y%m%dT%H%MZ)}"
OUTDIR="${2:-$HOME/fractal_sidecar_wikitext_rung_${STAMP}}"
SEEDS="${SEEDS:-0 1 2}"
LANES="${LANES:-attention rgrp tiny-mlp}"
STEPS="${STEPS:-1000}"
...
run_attention() {
  ...
  local run_name="wikitext-attention-seed${seed}-${STAMP}"
...
run_sidecar() {
  ...
  local run_name="wikitext-sidecar-${safe_type}-layer1-zero-seed${seed}-${STAMP}"
  "${PYTHON}" -m maxtext.trainers.pre_train.train \
    "${COMMON[@]}" \
    "steps=${steps}" \
    "run_name=${run_name}" \
  ...
  for lane in ${LANES}; do
    case "${lane}" in
      attention)
        run_attention "${seed}"
        ;;
      rgrp|tiny-mlp|tiny-glu|binary-tree)
